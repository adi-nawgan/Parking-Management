...
const MemberLayout: React.FC = () => {
  ...
  const menuItems: MenuItem[] = [
    ...
    { name: 'Search Plate', path: '/member/search-plate', icon: Search },
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-darkBg text-slate-800 dark:text-slate-100 flex flex-col md:flex-row transition-colors duration-300">
      <header className="md:hidden glass-panel flex justify-between items-center px-6 py-4 z-40 border-b border-slate-200/50 dark:border-white/5">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-brandTeal-500 to-emerald-500 flex items-center justify-center shadow-md">
            <Car className="w-4.5 h-4.5 text-white" />
          </div>
          <span className="font-extrabold text-lg tracking-wider bg-gradient-to-r from-brandTeal-500 to-emerald-500 bg-clip-text text-transparent">Member</span>
        </div>
      ...
      <aside className={`
        ...
        w-64 bg-[#070612] text-slate-200 border-r border-brandTeal-500/10 min-h-screen p-6 justify-between flex-shrink-0 shadow-2xl
      `}>
        <div>
          <div className="hidden md:flex items-center gap-3 mb-8 px-1">
            <div className="w-9 h-9 rounded-xl bg-gradient-to-tr from-brandTeal-500 to-emerald-500 flex items-center justify-center shadow-md">
              <Car className="w-5 h-5 text-white" />
            </div>
            <div className="flex flex-col">
              <span className="font-extrabold text-sm tracking-wider bg-gradient-to-r from-brandTeal-400 to-emerald-300 bg-clip-text text-transparent leading-none">Member Portal</span>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3.5 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 ${
                    isActive
                      ? 'bg-brandTeal-500/15 text-brandTeal-300 border-l-4 border-brandTeal-500 font-semibold'
                      : 'text-slate-400 hover:bg-white/5 hover:text-slate-100'
                  }`}
                >
                  <Icon className={`w-5 h-5 ${isActive ? 'text-brandTeal-400' : 'text-slate-400'}`} />
                  {item.name}
                </Link>
              );
            })}
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3.5 p-3 rounded-2xl bg-gradient-to-br from-darkCard/50 to-darkBg/30 border border-brandTeal-500/10 shadow-md">
            <div className="relative flex-shrink-0">
              <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-brandTeal-500 to-emerald-500 flex items-center justify-center text-white font-extrabold text-sm shadow-sm ring-2 ring-brandTeal-500/20">
            ...
            <div className="overflow-hidden flex-1 min-w-0">
              <p className="text-[9px] text-brandTeal-400 font-bold uppercase tracking-widest">Society Member</p>
